...
class LatentExplorerApp:
    def __init__(self, root, model, device, latent_dim=32, slider_range=(-3.0, 3.0), slider_step=0.1, imsize=256):
        self.root = root
        self.model = model
        self.device = device
        self.latent_dim = latent_dim
...
def main():
    ...
    root = tk.Tk()
    app = LatentExplorerApp(root, model=model, device=args.device, latent_dim=args.latent_dim, imsize=args.imsize)
    root.mainloop()
